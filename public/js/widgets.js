...
( function( $ ) {
	...
	$(document).ready(function($) {
		...
		$(document).on('click', 'input.wh-image-picker', function( event ){
			...
			custom_upload_frame_obj = wp.media.frames.custom_upload_frame_obj = wp.media({
				button: {
			...
			custom_upload_frame_obj.on( 'select', function() {

				var state = custom_upload_frame_obj.state('custom-state-insert');
